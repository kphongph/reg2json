...
module.exports = function (app, config) {
    ...
    */
    app.get(urlPrefix + '/:db/:collection/:id?', function(req, res) { 
        ...
        if (req.params.id) {
            query = {'_id': new ObjectID.createFromHexString(req.params.id)};
        }
        ...
        var test = ['limit','sort','fields','skip','hint','explain','snapshot','timeout','count'];
    ...
    */
    app.put(urlPrefix + '/:db/:collection/:id', function(req, res) {
        var spec = {'_id': new ObjectID.createFromHexString(req.params.id)};
    ...
    app.delete(urlPrefix + '/:db/:collection/:id', function(req, res) {
        ...
        connection.connect(dbConnectionStr + '/' + req.params.db, function (err, db) {
            ...
            db.collection(req.params.collection, function(err, collection) {
                ...
                collection.deleteOne(spec, function(err, docs) {
                    if (err) {
                        logger.error('Error removing from ' + collection + ': ' + err.message);
                        res.status(500).json({ message: 'Server error' });
                        return;
                    }
                    console.log('remove ',docs.deletedCount);
                    res.json({ ok: 1 });
                });
            });
        });
